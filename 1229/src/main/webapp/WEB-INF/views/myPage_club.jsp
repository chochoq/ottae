		...
		<div class="board_wrap01">
			...
			<script id="temp" type="text/x-handlebars-template">
				<thead>                                     
					<tr style="margin-bottom: 15px;">
						...
						<th class="c_opendate">승인 상태</th>
						<th class="c_del_button">탈퇴</th>
					</tr>
	            </thead>
            ...
            {{#each myclist}}
				<tr class=row c_code="{{c_code}}">
                	...
                	<td>{{c_regdate}}</td>
					<td>{{cu_schk}}</td>
                	<td><button class="del_one" id = "del_one">탈퇴</button></td>
            	</tr>
            	<hr>
            {{/each}}
            </script>
...
<script>
	...
	//체크박스선택삭제
 	$("#del_all").on("click", function() {
 		if (!confirm("선택한 동아리를 탈퇴하시겠습니까?")) return;
